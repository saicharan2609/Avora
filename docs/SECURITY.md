...
| ~~**SOQ-03**~~ | ~~What is the step-up mechanism when the only auth method is an email OTP — re-OTP, or a distinct second factor?~~ **Resolved by `AD-43` (2026-09-11, mechanism); addended `AD-43` (2026-09-11, factor and policy boundaries):** MFA/factor-based authentication is the decided security primitive; the V0 factor is **TOTP**, using the already-installed Supabase MFA capability — no new vendor. Also decided by the addendum: the enrollment boundary (TOTP is not required for ordinary login; a student must have an enrolled factor before completing a step-up-gated operation, guided through enrollment rather than bypassed) and the Avora-enforced step-up proof validity window (**5 minutes**, distinct from Supabase's own MFA challenge TTL). **Recovery policy (lost-factor/lost-device handling) remains undecided** — owner CISO, blocking for production readiness; a lost factor must not be bypassable via the ordinary login factor alone. Operation-specific binding and short validity remain Avora's own enforcement responsibility per `SEC-052`; MFA verification alone does not satisfy `SEC-052` by itself. `SOQ-02` (whether/when MFA is offered or required beyond step-up) remains separately open, not resolved by this entry or its addendum. | CTO (mechanism, factor, policy boundaries); CISO (recovery, open) | Re-OTP to a possibly-compromised inbox is a weaker step-up than `FR-002` implies for six irreversible operations |
