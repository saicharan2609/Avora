...
| 2026-08-26 | **Stage 12 Group 7 — Job/task naming correction: `resource.summary` → `summary.generate`** | Corrected a stale roadmap wording. This document's §6 Group 7 entry, deliverable #1, reads "Task declaration `resource.summary` in AI Gateway" — that name conflicts with `architecture.md` §24.1's job taxonomy, which names the same task **`summary.generate`** (trigger "Post-index", idempotency key `resource_id + prompt_version`). `architecture.md` is higher authority than this roadmap (`AGENTS.md` §2 / `CLAUDE.md`'s document authority table); the roadmap wording was stale, not the architecture. `summary.generate` is used throughout the Group 7 implementation — job/table names, AI Gateway task identifier, prompt/routing/telemetry code, and job payload column names. §6's Group 7 entry text is left unedited here (out of scope for this correction); this row is the authoritative record of the resolved name, mirroring how the Group 2 embedding-dimensionality correction above was recorded. | `packages/ai/gateway/invocation/SummaryInvocationPort.ts`, `packages/jobs/resource-summary/contracts.ts`, `packages/domain/notes/jobs/ResourceSummaryJob.contract.ts`, `supabase/migrations/20260826112000_resource_summary_jobs.sql`, `supabase/migrations/20260826113000_resource_summary_jobs_transactional_enqueue.sql` |
